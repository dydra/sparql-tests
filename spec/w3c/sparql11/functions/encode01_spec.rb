# coding: utf-8
#
require 'spec_helper'

# Auto-generated by build_w3c_tests.rb
#
# ENCODE_FOR_URI()
# 
# /Users/ben/Repos/datagraph/tests/tests/sparql11-tests/data-sparql11/functions/encode01.rq
#
# This is a W3C test from the DAWG test suite:
# http://www.w3.org/2001/sw/DataAccess/tests/r2#encode01
#
# This test is approved: 
# 
#
describe "W3C test" do
  context "functions" do
    before :all do
      @data = %q{
@prefix xsd: <http://www.w3.org/2001/XMLSchema#> .
@prefix : <http://example.org/> .

# numeric data
:n4 :num -2 .
:n1 :num -1 .
:n2 :num -1.6 .
:n3 :num 1.1 .
:n5 :num 2.5 .

# string data
:s1 :str "foo" .
:s2 :str "bar"@en .
:s3 :str "BAZ" .
:s4 :str "食べ物" .
:s5 :str "100%" .
:s6 :str "abc"^^xsd:string .
:s7 :str "DEF"^^xsd:string .

# date data
:d1 :date "2010-06-21T11:28:01Z"^^xsd:dateTime .
:d2 :date "2010-12-21T15:38:02-08:00"^^xsd:dateTime .
:d3 :date "2008-06-20T23:59:00Z"^^xsd:dateTime .
:d4 :date "2011-02-01T01:02:03"^^xsd:dateTime .

}
      @query = %q{
PREFIX : <http://example.org/>
PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
SELECT ?s ?str (ENCODE_FOR_URI(?str) AS ?encoded) WHERE {
	?s :str ?str
}

}
    end

    example "ENCODE_FOR_URI()", :w3c_status => 'unapproved' do
    
      graphs = {}
      graphs[:default] = { :data => @data, :format => :ttl}


      repository = 'functions-encode01'
      expected = [
          { 
              :encoded => RDF::Literal.new('foo' ),
              :s => RDF::URI('http://example.org/s1'),
              :str => RDF::Literal.new('foo' ),
          },
          { 
              :encoded => RDF::Literal.new('bar' ),
              :s => RDF::URI('http://example.org/s2'),
              :str => RDF::Literal.new('bar' , :language => 'en'),
          },
          { 
              :encoded => RDF::Literal.new('BAZ' ),
              :s => RDF::URI('http://example.org/s3'),
              :str => RDF::Literal.new('BAZ' ),
          },
          { 
              :encoded => RDF::Literal.new('%E9%A3%9F%E3%81%B9%E7%89%A9' ),
              :s => RDF::URI('http://example.org/s4'),
              :str => RDF::Literal.new('食べ物' ),
          },
          { 
              :encoded => RDF::Literal.new('100%25' ),
              :s => RDF::URI('http://example.org/s5'),
              :str => RDF::Literal.new('100%' ),
          },
          { 
              :encoded => RDF::Literal.new('abc' ),
              :s => RDF::URI('http://example.org/s6'),
              :str => RDF::Literal.new('abc' , :datatype => RDF::URI('http://www.w3.org/2001/XMLSchema#string')),
          },
          { 
              :encoded => RDF::Literal.new('DEF' ),
              :s => RDF::URI('http://example.org/s7'),
              :str => RDF::Literal.new('DEF' , :datatype => RDF::URI('http://www.w3.org/2001/XMLSchema#string')),
          },
      ]


      sparql_query(:graphs => graphs, :query => @query,       # unordered comparison in rspec is =~
                   :repository => repository, :form => :select).should =~ expected
    end
  end
end
