# coding: utf-8
#
require 'spec_helper'

# Auto-generated by build_w3c_tests.rb
#
# Basic - Prefix/Base 3
# 
# /Users/ben/repos/datagraph/tests/tests/data-r2/basic/base-prefix-3.rq
#
# This is a W3C test from the DAWG test suite:
# http://www.w3.org/2001/sw/DataAccess/tests/r2#base-prefix-3
#
# This test is approved: 
# http://lists.w3.org/Archives/Public/public-rdf-dawg/2007JulSep/att-0060/2007-08-07-dawg-minutes.html
#
# 20101219 jaa : bug indicator : parse fails

describe "W3C test" do
  context "basic" do
    before :all do
      @data = %q{
@prefix ns: <http://example.org/ns#> .
@prefix x:  <http://example.org/x/> .
@prefix z:  <http://example.org/x/#> .

x:x ns:p  "d:x ns:p" .
x:x x:p   "x:x x:p" .

z:x z:p   "z:x z:p" .

}
      @query = %q{
PREFIX ns: <http://example.org/ns#>
PREFIX x:  <http://example.org/x/>

SELECT * WHERE { x:x ns:p ?v } 

}
    end

    example "Basic - Prefix/Base 3", :status => 'bug' do
    
      graphs = {}
      graphs[:default] = { :data => @data, :format => :ttl}


      repository = 'basic-base-prefix-3'
      expected = [
          { 
              :v => RDF::Literal.new('d:x ns:p' ),
          },
      ]


      sparql_query(:graphs => graphs, :query => @query,       # unordered comparison in rspec is =~
                   :repository => repository, :form => :select).should =~ expected
    end
  end
end
